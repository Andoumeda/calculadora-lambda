...
(define (inicio)
  (cond [(empty? (imprimir (evaluar (validar-sintaxis (leer))))) null]
        [else inicio]))
...
; (Es la estructura lo que importa no el nombre de las variables!)
...
(define (reduccion-beta expresion)
  (cond [(empty? (reduccion-beta-aux expresion)) null]
